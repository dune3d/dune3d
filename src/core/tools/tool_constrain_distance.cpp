...
ToolBase::CanBegin ToolConstrainDistance::can_begin()
{
    ...
    else if (lp && any_of(m_tool_id, ToolID::CONSTRAIN_DISTANCE, ToolID::MEASURE_DISTANCE)) {
        if (m_tool_id == ToolID::MEASURE_DISTANCE)
            return true;
    ...
    else if (lps && any_of(m_tool_id, ToolID::CONSTRAIN_DISTANCE, ToolID::MEASURE_DISTANCE)) {
        if (m_tool_id == ToolID::MEASURE_DISTANCE)
            return true;
...
ToolResponse ToolConstrainDistance::begin(const ToolArgs &args)
{
    ...
    if (tp) {
        ...
        constraint.m_measurement = m_tool_id == ToolID::MEASURE_DISTANCE;
    }
    else if (lps) {
